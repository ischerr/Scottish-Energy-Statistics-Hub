...
GasConsumptionHHoldOutput <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(column(8,
...
###### Server ######
GasConsumptionHHold <- function(input, output, session) {
  ...
  output$Text <- renderUI({
    tagList(column(12,
                   HTML(
                     paste(readtext("Structure/4 - Energy Efficiency/Demand Reduction/GasConsumptionHHold.txt")[2])
