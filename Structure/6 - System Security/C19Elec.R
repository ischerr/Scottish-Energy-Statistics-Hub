...
C19ElecOutput <- function(id) {
  ...
  tagList(
    tabsetPanel(
      tabPanel("Daily demand",
               fluidRow(column(8,
                               ...
                               h4(textOutput(ns('C19DailySubtitle')), style = "color: #5d8be1;")
               ),
               column(
                 4, style = 'padding:15px;',
                 downloadButton(ns('C19Daily.png'), 'Download Graph', style="float:right")
               )),
               
               tags$hr(style = "height:3px;border:none;color:#5d8be1;background-color:#5d8be1;"),
               #dygraphOutput(ns("C19ElecPlot")),
               plotlyOutput(ns("C19DailyPlot"), height = "600px")%>% withSpinner(color="#5d8be1"),
      ...
      tabPanel("Daily demand - Rolling Average",
               fluidRow(column(8,
                               h3("Daily electricity demand, 7-day rolling average", style = "color: #5d8be1;  font-weight:bold"),
                               h4(textOutput(ns('C19RollingSubtitle')), style = "color: #5d8be1;")
               ),
               column(
                 4, style = 'padding:15px;',
                 downloadButton(ns('C19Rolling.png'), 'Download Graph', style="float:right")
               )),
               
               tags$hr(style = "height:3px;border:none;color:#5d8be1;background-color:#5d8be1;"),
               #dygraphOutput(ns("C19ElecPlot")),
               plotlyOutput(ns("C19RollingPlot"), height = "600px")%>% withSpinner(color="#5d8be1"),
               HTML("<blockquote><p>Dates taken from the Scottish Parliament Information Centre <a href='https://spice-spotlight.scot/2021/01/08/timeline-of-coronavirus-covid-19-in-scotland/' target='_blank' rel='noopener'>Timeline of Coronavirus</a></p></blockquote>"),
               tags$hr(style = "height:3px;border:none;color:#5d8be1;background-color:#5d8be1;")
      ),
    ...
    tabPanel("Daily demand",
             ...
             fluidRow(
               column(12, dataTableOutput(ns("C19DailyTable"))%>% withSpinner(color="#5d8be1"))),
             tags$hr(style = "height:3px;border:none;color:#5d8be1;background-color:#5d8be1;")),
...
C19Elec <- function(input, output, session) {
  ...
  output$C19DailySubtitle <- renderText({
    
    paste("Scotland, 2019 - 2021")
  })
  
  output$C19DailyPlot <- renderPlotly  ({
    library(readr)
    library(ISOweek)
    library(lubridate)
    library(zoo)
  ...
  output$C19RollingSubtitle <- renderText({
    
    paste("Scotland, 2019 - 2021")
  })
  
  output$C19RollingPlot <- renderPlotly  ({
    library(readr)
    library(ISOweek)
    library(lubridate)
    library(zoo)
    ...
    DailyDemand$Electricity <- rollmean(DailyDemand$Electricity, 7, fill = NA, align = "right")
    ...
    p2 <-  plot_ly(DailyDemand,x = ~ Date ) %>% 
      ...
      add_trace(data = DailyDemand,
                ...
                legendgroup = "3",
                text = paste0(
                  "7-Day Average Electricity Demand: ",
                  round(DailyDemand$`2019`, digits = 1),
                  " GWh\nEnding: ", weekdays(ymd(paste0("2019-",substr(DailyDemand$NewDate,6,10)))), " ",
                  format(ymd(paste0("2019-",substr(DailyDemand$NewDate,6,10))), format="%d %B %Y")
                ),
                hoverinfo = 'text',
                colour = ChartColours[3],
      ...
      add_trace(data = DailyDemand,
                ...
                text = paste0(
                  "7-Day Average Electricity Demand: ",
                  round(DailyDemand$`2020`, digits = 1),
                  " GWh\nEnding: ", weekdays(ymd(paste0("2020-",substr(DailyDemand$NewDate,6,10)))), " ",
                  format(ymd(paste0("2020-",substr(DailyDemand$NewDate,6,10))), format="%d %B %Y")
                ),
                hoverinfo = 'text',
                colour = ChartColours[2],
      ...
      add_trace(data = DailyDemand,
                ...
                text = paste0(
                  "7-Day Average Electricity Demand: ",
                  round(DailyDemand$`2021`, digits = 1),
                  " GWh\nEnding: ", weekdays(ymd(paste0("2021-",substr(DailyDemand$NewDate,6,10)))), " ",
                  format(ymd(paste0("2021-",substr(DailyDemand$NewDate,6,10))), format="%d %B %Y")
                ),
                hoverinfo = 'text',
                colour = ChartColours[1],
  ...
  output$C19DailyTable = renderDataTable({
...
output$C19Daily.png <- downloadHandler(
...
output$C19Rolling.png <- downloadHandler(
  filename = "C19Rolling.png",
  content = function(file) {
    library(readr)
    library(ISOweek)
    library(lubridate)
    ...
    plottitle = "Daily electricity demand, rolling average"
    sourcecaption = "Source: National Grid, SPICe"
