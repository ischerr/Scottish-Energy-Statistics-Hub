  ...
  output$C19GasPlot <- renderPlotly  ({
    ...
    p1 <-  plot_ly(WeekdayElecDemand, x = ~ YearFormat ) %>%  
      ...
      add_trace(y = ~ `PostLockdown`, 
                name = "Fourth week of March to second week of November",
                type = 'bar',
                legendgroup = "2",
                text = paste0(
                  "Average weekday gas consumption in from the fourth week in March to second week of November: ", format(round(WeekdayElecDemand$`PostLockdown`, 0.1), big.mark = ",")," GWh\n",
                  "Year: ", WeekdayElecDemand$Year, "\n"),
                hoverinfo = 'text',
                line = list(width = 4)
      ) %>% 
      ...
      WeekdayElecDemandChart <- WeekdayElecDemand  %>%
        ...
        scale_fill_manual(
          ...
          values = c(
            ...
            "4th wk. Mar. to 2nd wk. Nov." = BarColours[2]
          )
        ) +
