...
RenewableSourcesOutput <- function(id) {
  ns <- NS(id)
  tagList(
    tabsetPanel(
      tabPanel("Displacement",
    fluidRow(column(8,
...
###### Server ######
RenewableSources <- function(input, output, session) {
  ...
  output$Text <- renderUI({
    tagList(column(12,
                   HTML(
                     paste(readtext("Structure/2 - Renewables/Electricity/RenewableSources.txt")[2])
