...
xDisplacedEmissionsOutput <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(column(8,
                    h3("Estimated million tonnes of CO2 emissions displaced by renewables", style = "color: #39ab2c;  font-weight:bold"),
                    h4(textOutput(ns('xDisplacedEmissionsSubtitle')), style = "color: #39ab2c;")
    ),
             column(
               4, style = 'padding:15px;',
               downloadButton(ns('xDisplacedEmissions.png'), 'Download Graph', style="float:right")
             )),
    
    tags$hr(style = "height:3px;border:none;color:#39ab2c;background-color:#39ab2c;"),
    #dygraphOutput(ns("xDisplacedEmissionsPlot")),
    plotlyOutput(ns("xDisplacedEmissionsPlot"))%>% withSpinner(color="#39ab2c"),
    tags$hr(style = "height:3px;border:none;color:#39ab2c;background-color:#39ab2c;"),
    fluidRow(
    ...
    fluidRow(
      column(12, dataTableOutput(ns("xDisplacedEmissionsTable"))%>% withSpinner(color="#39ab2c"))),
    tags$hr(style = "height:3px;border:none;color:#39ab2c;background-color:#39ab2c;"),
    fluidRow(
      column(1,
             p("Next update:")),
...
###### Server ######
xDisplacedEmissions <- function(input, output, session) {
  ...
  output$xDisplacedEmissionsSubtitle <- renderText({
  ...
  output$xDisplacedEmissionsPlot <- renderPlotly  ({
  ...
  output$xDisplacedEmissionsTable = renderDataTable({
  ...
  output$Text <- renderUI({
    tagList(column(12,
                   HTML(
                     paste(readtext("Structure/8 - Greenhouse Gases/xDisplacedEmissions.txt")[2])
                     
                   )))
  })
  
  observeEvent(input$ToggleTable, {
    toggle("xDisplacedEmissionsTable")
  })
  ...
  output$xDisplacedEmissions.png <- downloadHandler(
    filename = "xDisplacedEmissions.png",
