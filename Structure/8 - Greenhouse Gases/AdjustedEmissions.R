...
AdjustedEmissions <- function(input, output, session) {
  ...
  print("AdjustedEmissions.R")
    ...
    output$AdjustedEmissionsPercentageReductionTargetsSubtitle <- renderText({
      ...
      paste("Scotland,", max(AdjustedEmissions$Year),"-", max(AdjustedEmissions$Year))
    })
  ...
  output$AdjustedEmissionsPercentageReductionTargetsPlot <- renderPlotly  ({
    ...
    p <-  plot_ly(data = AdjustedEmissions,
      ...
      add_trace(data = AdjustedEmissions,
                ...
                text = paste0(
                  "1998 - 2016 Inventory: ",
                  round(AdjustedEmissions$`Total`, digits = 1),
                  " MtCO2e\nYear: ",
      ...
      add_trace(
        data = AdjustedEmissions[which(AdjustedEmissions$Year %in% c(ymd("1990-01-01"),ymd("1995-01-01"),ymd("1998-01-01"),max(AdjustedEmissions[which(AdjustedEmissions$Total != 0),]$Year))),],
        x = ~ Year,
        y = ~ `Total`,
        name = "Total greenhouse gas emissions",
        text = paste0(
          "1998 - 2016 Inventory: ",
          round(AdjustedEmissions[which(AdjustedEmissions$Year %in% c(ymd("1990-01-01"),ymd("1995-01-01"),ymd("1998-01-01"),max(AdjustedEmissions[which(AdjustedEmissions$Total != 0),]$Year))),]$`Total`, digits = 1),
          " MtCO2e\nYear: ",
          format(AdjustedEmissions[which(AdjustedEmissions$Year %in% c(ymd("1990-01-01"),ymd("1995-01-01"),ymd("1998-01-01"),max(AdjustedEmissions[which(AdjustedEmissions$Total != 0),]$Year))),]$Year, "%Y")
        ),
        hoverinfo = 'text',
        showlegend = FALSE ,
        type = "scatter",
  ...
  output$Text <- renderUI({
    tagList(column(12,
                   HTML(
                     paste(readtext("Structure/8 - Greenhouse Gases/AdjustedEmissions.txt")[2])
