...
C19Gas <- function(input, output, session) {
  ...
  output$C19GasPlot <- renderPlotly  ({
    ...
    p1 <-  plot_ly(WeekdayElecDemand, x = ~ YearFormat ) %>%  
      ...
      add_trace(y = ~ `PostLockdown`, 
                name = "Fourth week of March to second week of August",
                type = 'bar',
                legendgroup = "2",
                text = paste0(
                  "Average weekday gas consumption in from the fourth week in March to second week of August: ", format(round(WeekdayElecDemand$`PostLockdown`, 0.1), big.mark = ",")," GWh\n",
                  "Year: ", WeekdayElecDemand$Year, "\n"),
                hoverinfo = 'text',
                line = list(width = 4)
      ) %>% 
  ...
  output$C19GasTable = renderDataTable({
    ...
    names(WeekdayElecDemand) <- c("Year", "First three weeks of March (GWh)", "Fourth week of March to second week of August (GWh)")
    datatable(
      WeekdayElecDemand,
      extensions = 'Buttons',
      ...
      WeekdayElecDemandChart <- WeekdayElecDemand  %>%
        ...
        scale_fill_manual(
          ...
          values = c(
            ...
            "4th week Mar. to 2nd week Aug." = BarColours[2]
          )
        ) +
