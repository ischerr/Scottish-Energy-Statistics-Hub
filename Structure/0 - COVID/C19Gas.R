...
C19Gas <- function(input, output, session) {
  ...
  output$C19GasPlot <- renderPlotly  ({
    ...
    p1 <-  plot_ly(WeekdayElecDemand, x = ~ YearFormat ) %>%  
      ...
      add_trace(y = ~ `PostLockdown`, 
                ...
                legendgroup = "2",
                text = paste0(
                  "Average weekday gas consumption in from the last week in March to second week of June: ", format(round(WeekdayElecDemand$`PostLockdown`, 0.1), big.mark = ",")," GWh\n",
                  "Year: ", WeekdayElecDemand$Year, "\n"),
                hoverinfo = 'text',
                line = list(width = 4)
      ) %>% 
  ...
  output$C19Gas.png <- downloadHandler(
    ...
    content = function(file) {
      ...
      DailyDemand$PostLockdown <- ifelse(DailyDemand$Week >= 13, "First three weeks of March", "Last week of March to second week of June")
      ...
      WeekdayElecDemandChart <- WeekdayElecDemand  %>%
        ...
        scale_fill_manual(
          ...
          values = c(
            ...
            "Last week of March to second week of June" = BarColours[2]
          )
        ) +
