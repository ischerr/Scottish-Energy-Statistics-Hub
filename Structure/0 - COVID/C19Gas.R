...
C19Gas <- function(input, output, session) {
  ...
  output$C19GasPlot <- renderPlotly  ({
    ...
    p1 <-  plot_ly(WeekdayElecDemand, x = ~ YearFormat ) %>%  
      ...
      add_trace(y = ~ `PostLockdown`, 
                name = "Fourth week of March to fourth week of July",
                type = 'bar',
                legendgroup = "2",
                text = paste0(
                  "Average weekday gas consumption in from the fourth week in March to fourth week of July: ", format(round(WeekdayElecDemand$`PostLockdown`, 0.1), big.mark = ",")," GWh\n",
                  "Year: ", WeekdayElecDemand$Year, "\n"),
                hoverinfo = 'text',
                line = list(width = 4)
      ) %>% 
  ...
  output$C19GasTable = renderDataTable({
    ...
    names(WeekdayElecDemand) <- c("Year", "First three weeks of March (GWh)", "Fourth week of March to fourth week of July (GWh)")
    datatable(
      WeekdayElecDemand,
      extensions = 'Buttons',
  ...
  output$C19Gas.png <- downloadHandler(
    ...
    content = function(file) {
      ...
      DailyDemand$PostLockdown <- ifelse(DailyDemand$Week >= 13, "First three weeks of March", 
                                                                 "4th week March to 2nd week July")
      ...
      WeekdayElecDemandChart <- WeekdayElecDemand  %>%
        ...
        scale_fill_manual(
          ...
          values = c(
            ...
            "4th week March to 2nd week July" = BarColours[2]
          )
        ) +
