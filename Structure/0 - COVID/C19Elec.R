  ...
  output$C19ElecPlot <- renderPlotly  ({
    ...
    p1 <-  plot_ly(WeekdayElecDemand, x = ~ Year ) %>%  
      ...
      add_trace(y = ~ `PostLockdown`,
                ...
                legendgroup = "2",
                text = paste0(
                  "Average weekday electricity consumption in from the last week in March to first week of June: ", format(round(WeekdayElecDemand$`PostLockdown`, 0.1), big.mark = ",")," GWh\n",
                  "Year: ", WeekdayElecDemand$Year, "\n"),
                hoverinfo = 'text',
                line = list(width = 4)
      ) %>% 
