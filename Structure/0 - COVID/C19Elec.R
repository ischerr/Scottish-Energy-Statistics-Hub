...
C19Elec <- function(input, output, session) {
  ...
  output$C19ElecPlot <- renderPlotly  ({
    ...
    p1 <-  plot_ly(WeekdayElecDemand, x = ~ Year ) %>%  
      ...
      add_trace(y = ~ `PostLockdown`,
                name = "Last week of March to fourth week in May",
                type = 'bar',
                legendgroup = "2",
                text = paste0(
                  "Average weekday electricity consumption in from the last week in March to fourth week of May: ", format(round(WeekdayElecDemand$`PostLockdown`, 0.1), big.mark = ",")," GWh\n",
                  "Year: ", WeekdayElecDemand$Year, "\n"),
                hoverinfo = 'text',
                line = list(width = 4)
      ) %>% 
  ...
  output$C19ElecTable = renderDataTable({
    ...
    names(WeekdayElecDemand) <- c("Year", "First three weeks of March (GWh)", "Last week of March to fourth week in May (GWh)")
    datatable(
      WeekdayElecDemand,
      extensions = 'Buttons',
  ...
  output$C19Elec.png <- downloadHandler(
    ...
    content = function(file) {
      ...
      DailyDemand$PostLockdown <- ifelse(DailyDemand$Week >= 13, "First three weeks of March", "Last week of March to fourth week in May")
      ...
      WeekdayElecDemandChart <- WeekdayElecDemand  %>%
        ...
        scale_fill_manual(
          ...
          values = c(
            ...
            "Last week of March to fourth week in May" = BarColours[2]
          )
        ) +
